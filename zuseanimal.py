...
async def useanimal(message, animal, client, thing):
	...
	if randomrea == 1: amount = int(round(amount*0.8))
	if randomrea == 2: amount = int(round(amount*0.9))
	...
	if echance ==2:
		...
		if whatthingchance == 2:
			if 'lastbred' in db['members'][str(message.author.id)]['animals'][animal]:
				if db['members'][str(message.author.id)]['animals'][animal]['lastbred'] + 3600000 > now:
					return
			if amount < 8:
				await message.reply(f'{toolbreak}\nYou {thing}ed `{amountr}` `{animal}(s)`, gaining `{amount}` `{merch}`(s). Wait `{cooldown}` seconds before {thing}ing your {animal} again')
				return
			...
			if randomrar == 1:
				...
				thingtotype = random.choice(births)
				animales = ['https://opensanctuary.org/wp-content/uploads/2018/04/Bitsy-sheep.jpg', 'https://upload.wikimedia.org/wikipedia/commons/thumb/0/0c/Cow_female_black_white.jpg/1200px-Cow_female_black_white.jpg', 'https://4-hontario.ca/wp-content/uploads/2020/09/Poultry_Main-Image-1626x1080.jpeg']
				...
				if res.author == message.author:
					if res.component.label == 'Sheep' and animale == 'https://opensanctuary.org/wp-content/uploads/2018/04/Bitsy-sheep.jpg' or res.component.label == 'Chicken' and animale == 'https://4-hontario.ca/wp-content/uploads/2020/09/Poultry_Main-Image-1626x1080.jpeg' or res.component.label == 'Cow' and animale == 'https://upload.wikimedia.org/wikipedia/commons/thumb/0/0c/Cow_female_black_white.jpg/1200px-Cow_female_black_white.jpg':
						bornedamount = random.randint(1, round(amount/5))
						if amount > 100:
						...
						await message.channel.send(f'oop {bornedamount} {animal}(s) were born')
						a = db['members']
						a[str(message.author.id)]['animals'][animal]['amount'] += bornedamount
						a[str(message.author.id)]['animals'][animal]['lastbred'] = now
						db['members'] = a
