...
async def beach(message, client):
    ...
    if user not in db["members"]:
        await message.channel.send("make an account to search the beach")
        return
    ...
    if thing == 1:
        ...
        await message.channel.send(
            f"On the way over to the beach, you accidentally smelled a really smelly {thing2} and died. you paid 100 coins to be reborn."
        )
        a = db["members"]
    ...
    if item[0] == "dragonegg":
        await message.channel.send(
            f"You looked at the beach, and there, in a pile of sand, you saw it. A small piece of ginseng.\n{durabilityMsg}\n{reserveMsg}"
        )

    elif item[0] == "nothing":
        await message.channel.send(
            f"You found nothing on the beach\n{durabilityMsg}\n{reserveMsg}"
        )

    elif item[0] == "death":
        await message.channel.send(
            "You were searching the beach when, all of a sudden, a huge horde of seagulls swooped down on you and pecked you to death. you paid 100 coins to be reborn."
        )
        m = db["members"]
