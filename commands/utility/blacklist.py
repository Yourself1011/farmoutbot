...
async def blacklist(message, client):
	...
	if args[2].lower() == 'add':
		...
		return await message.reply(f'{user.name} was added to {args[3].lower()} blacklist')	
	...
	if args[2].lower() == 'remove':
		...
		if not user or not user in db['blacklist'][args[3].lower()]: return 'thats not a person that\'s blacklisted'
		...
		return await message.reply(f'{user.name} was removed from {args[3].lower()} blacklist')