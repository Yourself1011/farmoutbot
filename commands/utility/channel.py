    ...
    if args[2].lower() == "show":
        if channel == None:
            await message.channel.send(
                f"`{name}` has not set a system messages channel."
            )
            return
        await message.channel.send(
    ...
    if args[2].lower() == "set":
        if not message.author.guild_permissions.manage_guild:
            ...
            if len(args) == 2:
                ...
                return
            if (
                not args[2].startswith("<#")
                and args[2].endswith(">")
                and not args[2].lower() == "none"
            ):
                await message.channel.send(
                    "mention a channel for this to work, or say none"
                )
                return
