...
async def beetlerace(message, client):
	...
	if len(args) == 2:
		await message.channel.send('please specify which beetle you are racing, and how much you bet for him to win')
		return
	if not args[2].isnumeric() or not args[3].isnumeric():
		await message.channel.send('ur dumb both the beetle number and the amount bet have to be integers greater than 0')
		return
	...
	while b1 < 25 and b2 < 25 and b3 < 25 and b4 < 25 and b5 < 25:
		...
		if b1 >= 25 or b2 >= 25 or b3 >= 25 or b4 >= 25 or b5 >= 25:
			break
	if args[2] == '1': beetle = b1
	if args[2] == '2': beetle = b2
	if args[2] == '3': beetle = b3
	if args[2] == '4': beetle = b4
	if args[2] == '5': beetle = b5
	for i in beetles:
		if i >= 25:
			beetlewon = i
	...
	e.set_author(name=f'{message.author.name}\'s beetle race:', icon_url=message.author.avatar_url)
	for i in beetles:
		if i == beetle:
			beetles[i] = f'YOUR BEETLE: {beetle}'
		beetles[i] = f'{beetles[i]} :beetle:'
	e.add_field(name = 'Beetles', value = f'1st: {beetles[0]}\n2nd: {beetles[1]}\n3rd: {beetles[2]}\n4th: {beetles[3]}\n5th: {beetles[4]}')
	if beetle == beetles[0]: 
		a = db['members']
