...
async def changeprefix(message, client):
	...
	if len(args) == 2:
		await message.channel.send('gotta specify a new prefix dummo, make sure it has no space')
		...
		await message.channel.send(f':white_check_mark: the prefix for `{name}` has been changed from `{oldprefix}` to `{newprefix}` by {message.author.mention}.')