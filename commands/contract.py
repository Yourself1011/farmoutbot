...
async def contract(message, client):
    ...
    if len(args) == 2:
        await message.channel.send(
            "do a subcommand: `show`, `sign`, `complete`, or `current`"
        )
        return
    ...
    for i in db["members"][str(message.author.id)]["donecontracts"][0]:
        if len(i) < 3:
            ...
            break
    if not currentpart:
        await message.channel.send("you've already done all the contracts lol")
        return
    ...
    if command == "sign":
        currentpart = str(currentpart)
        if len(args) == 3:
            await message.channel.send("which one are you signing lol")
            return
        if not args[3].isnumeric():
            await message.channel.send(
                "gotta say a number corresponding to the contract id"
            )
            return
        signed = int(args[3])
        ...
        if (
            ...
            await message.channel.send("oy you've already done that contract")
            return
        if (
            not db["members"][str(message.author.id)]["currentcontract"] == []
            and signed == db["members"][str(message.author.id)]["currentcontract"][1]
            and currentpart
            == db["members"][str(message.author.id)]["currentcontract"][0]
        ):
            await message.channel.send("you're doing that contract right now idot")
            return
        a = db["members"]
    ...
    if command == "complete":
        ...
        if (
            ...
            await message.channel.send(
                "you dont have what you need to complete the contract"
            )
            return
        a = db["members"]
        ...
        if currentpart == 4 and completed == 3:
            await message.channel.send(
                "woah you did all the contracts already? i better add more soon lol"
            )
            return
        else:
            await message.reply(
