...
async def beg(message, client):
	...
	if db['members'][str(message.author.id)]['reputation'] <= 250:
		await message.channel.send(f'{message.author.mention} Your reputation is too low, it has to be at least `250` for this command to work.')
		return
	now = int(round(time.time()*1000))
	if 'beg' in db['members'][str(message.author.id)]['cooldowns'] and db['members'][str(message.author.id)]['cooldowns']['beg']<now:
		now = int(round(time.time()*1000))
