...
async def gift(message, client):
	...
	if str(user) not in db['members']:
		await message.channel.send(f'Get {userObj.name}#{userObj.discriminator} to make an account')
		return
	...
	thinggifted = softSearch(list(animals.keys()) + list(tools.keys()) + list(seeds.keys()) + list(merch.keys()), args[3].lower(), ignore=["name"]) #kekw hey i thought you were adding softSearch Me??? fine
	...
	await message.channel.send(f'{message.author.mention} gave {name} `{amount} {thinggifted}(s)`.')
