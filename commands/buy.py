...
async def buy(message, client):
    ...
    if len(args) == 4 and args[3] in ["a", "all", "max"]:
        ...
        if args[3] == "0":
            await message.channel.send(
                "you bought 0 things. are you proud of yourself?"
            )
            return
    ...
    if cost * amount > db["members"][str(message.author.id)]["money"]:
        await message.channel.send(
            f" you're too poor to buy `{amount} {key}(s)` at `{cost}` each."
        )
        return
    if key in tools:
        amount = 1
    ...
    async def buye():
        ...
        if thingi == 1:
            ...
            await message.channel.send(
                f"{message.author.mention} you lucky ducky, you bought something that the market was trying to get rid of and your rep increased by `{thingj}`."
            )
            a = db["members"]
        ...
        if thingi == 2:
            thingj = random.randint(1, 5)
            await message.channel.send(
                f"{message.author.mention} bruh you bought something that the market didnt want to give, you lost `{thingj}` rep **and** `{int(round(thingj*2))} coins`."
            )
            a = db["members"]
