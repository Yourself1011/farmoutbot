...
async def collect(message, client):
    ...
    if (
        ...
        await message.channel.send(
            f" your plant isn't ready yet, wait `{str(e)}` seconds dumbo"
        )
        return
    ...
    if (
        ...
        await message.channel.send(
            f"Idiot your {plant}(s) couldn't live there and {amountLost} died."
        )
