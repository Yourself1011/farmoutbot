...
async def share(message, client):
	...
	if amount > db['members'][str(message.author.id)]['money']:
		await message.channel.send('that\'s more money than you have, learn to count dummy')
		return
	...
	await message.channel.send(f'{message.author.mention} gave `{amount}` coins to {person.name}. ')