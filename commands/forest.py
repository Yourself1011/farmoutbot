...
async def forest(message, client):
    ...
    if user not in db["members"]:
        await message.channel.send("make an account to search the forest")
        return
    ...
    if thing == 1:
        ...
        await message.channel.send(
            f"On the way over to the forest, you accidentally saw a weird {thing2} and died. you paid 100 coins to be reborn."
        )
        a = db["members"]
    ...
    if not bool(randint(0, 7)):
        ...
        repLoss = randint(
            floor(user["reputation"] / 50), floor(user["reputation"] / 10)
        )
    ...
    if item[0] == "dragonegg":
        await message.channel.send(
            f"You looked in the forest, and there, in a clearing, you saw it. {'A' if item[1] == 1 else 'Two'} dragon egg{'s' if item[1] == 2 else ''}\n{durabilityMsg}\n{reserveMsg}"
        )

    elif item[0] == "nothing":
        await message.channel.send(
            f"You found nothing in the forest\n{durabilityMsg}\n{reserveMsg}"
        )

    else:
        responses = [
            "looked in the forest and found",
            "walked around a little and picked up",
            "discovered",
            "picked",
            "looked in the stump of a dead tree and found",
        ]
        await message.channel.send(
