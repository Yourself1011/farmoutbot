...
async def eat(message, client):
	...
	if eated == 'cake':
		await message.channel.send(
			'you ate your cake, and you got another cake.')
		a = db['members']
	...
	if "loottable" in merch[eated]:
		...
		items = gatheringCmd(message, merch[eated]["loottable"], [merch[eated]["amount"][0] * repeat, merch[eated]["amount"][1] * repeat, merch[eated]["amount"][2]])
		...
		for i in items:
			...
			if i[0] in user[
					itemType] and itemType != "tools" and itemType != "merch":
				...
				user["tools"][i[0]]["durability"] = tools[
					i[0]]["durability"]
		...
		verb2 = [
			"out came", "received", "got", "farted out", "burped out", "found",
			"out of thick ear appeared", "a ufo came and dropped off"
		]
