...
async def profile(message, client):
	...
	for i in db['members'][user]['animals']:
		if type(animals[i]['cost']) == int: totalworth += animals[i]['cost']
	for i in db['members'][user]['tools']:
		if type(tools[i]['cost']) == int: totalworth += tools[i]['cost']
	for i in db['members'][user]['merch']:
		if type(merch[i]['cost']) == int: totalworth += merch[i]['cost']
	for i in db['members'][user]['seeds']:
		if type(seeds[i]['cost']) == int: totalworth += seeds[i]['cost']
	totalworth += money

	reputation = db['members'][user]['reputation']
	e.add_field(name = '- :moneybag: Currency:', value = f'coins: {money}\ntotal worth: {totalworth}\nreputation: {reputation}', inline = False)
	...
	e.add_field(name = '- :1234: Totals:', value = f'animals: {totalanimals}\ntools: {totaltools}\nmerch: {totalmerch}', inline = False)
	...
	e.add_field(name = '- :bar_chart: Other:', value = f'shared: {shared}\ngambled: {gambled}\nbought: {bought}\nsold: {sold}\nanimals used: {used}\ncommands used: {commands}', inline = False)
