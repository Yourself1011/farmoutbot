...
async def devmode(message, client):
    ...
    if (
        ...
        and not environ.get("ENV") == "dev"
    ):
        responses = [
            "Thank you for using Farmout. All of our agents are currently unavailable. Our next available agent will be with you shortly. *elevator music plays*",
            "uhm no",
            "Be a dev smh",
            "Wait five more minutes. If I still don't work, read this message again.",
            "Shh... I'm sleeping. Don't disturb me",
            "bruh",
            "Maybe I shouldn't be focusing on adding more of these responses and start finishing the bot",
            ...
            "you missed your chance to be a beta tester 2bad4u",
            "You're fat. Did you know that?",
            "You're dumb. Like a babbon bum?",
            "why are we spending so much time on this we'll just remove it eventually anyway",
    ...
    if args[2] == "toggle":
        ...
        if f"{str(message.author.id)} real" in members:
            ...
            members[str(message.author.id)] = {
                ...
                "location": "default",
                "locations": {},
                "settings": {"votedm": True, "tips": True},
            }
        ...
        members[str(message.author.id)] = updateDict(
            db["members"][str(message.author.id)],
            loads("".join(args[3 : len(args)]).replace("'", '"')),
        )
        ...
        members[str(user.id)] = updateDict(
            db["members"][str(user.id)],
            loads("".join(args[4 : len(args)]).replace("'", '"')),
        )
