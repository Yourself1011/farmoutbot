...
async def listtrades(message, client):
	...
	for i in range(len(db['trades'])):
		...
		e.add_field(name = f'{i} (Stock: {user["trades"]["stock"][i] - user["trades"]["tradeAmts"][i]}): ', value = f'{"~~" if user["trades"]["tradeAmts"][i] >= user["trades"]["stock"][i] else ""}*Give (trade value: {giveVal}):* {", ".join(list(map(mapFunc, give)))}\n*Get (trade value: {getVal}):* {", ".join(list(map(mapFunc, get)))}{"~~" if user["trades"]["tradeAmts"][i] >= user["trades"]["stock"][i] else ""}', inline = False)

	prefix = db['server'][str(message.guild.id)]['prefix']
	e.set_footer(text=f'Use <{prefix} trade (trade number)> to request a trade. If your reputation is too low, the trade may be rejected.')
