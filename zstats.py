...
def gatheringCmd(msg, loottable):
    ...
    itemKey = min(
        {k: v for k, v in newList.items() if v[0] >= chosen}.items(),
        key=lambda x: x[1][0],
    )[0]
...
merch = {
    ...
    "pebble": {"name": "pebble :rock:", "cost": 5, "sellcost": 3, "tradevalue": 5},
    "voldysnose": {
    ...
    "bread": {"name": "bread :bread:", "cost": 20, "sellcost": 16, "tradevalue": 15},
    "umbrella": {
    ...
    "cactus": {"name": "cactus :cactus:", "cost": 35, "sellcost": 32, "tradevalue": 20},
    # Gather only merch
    "mushroom": {
